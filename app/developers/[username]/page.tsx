...
export default function DeveloperProfilePage({ params }: Props) {
  ...
  useEffect(() => {
    void loadData(username);
  }, [username]);

  async function loadData(nextUsername: string) {
    setLoading(true);
    setError("");

    try {
      const profileData = await fetchDeveloperProfile(nextUsername);
      const repoData = await fetchAllRepos(nextUsername, profileData.public_repos);
  ...
  async function fetchAllRepos(nextUsername: string, expectedTotal: number) {
    const perPage = 100;
    let page = 1;
    const allRepos: GithubRepository[] = [];

    while (true) {
      const pageRepos = await fetchDeveloperRepos(nextUsername, page, perPage);
      allRepos.push(...pageRepos);
      if (pageRepos.length < perPage || allRepos.length >= expectedTotal) break;
      page += 1;
    }

    return allRepos.slice(0, expectedTotal);
  }
        ...
        <div className="mt-4 space-y-6">
          ...
          <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div className="flex items-end gap-4 -mt-10">
                  <Image
                    ...
                    width={88}
                    height={88}
                    className="h-16 w-16 rounded-xl border-4 border-white shadow-md sm:h-[88px] sm:w-[88px]"
                  />
                  <div className="pb-1">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:pb-1">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:px-4 sm:py-2 sm:text-sm"
                  >
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors sm:px-4 sm:py-2 sm:text-sm ${
                      shortlisted
                  ...
                  {shortlisted && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:py-2 sm:text-sm"
                    >
            ...
            <div className="grid grid-cols-3 gap-px border-t border-gray-200 bg-gray-100">
              <div className="bg-white px-2 py-3 text-center sm:px-4 sm:py-4">
                <p className="text-lg font-bold text-gray-900 sm:text-xl">{profile.public_repos}</p>
                <p className="text-[10px] text-gray-500 sm:text-xs">Repositories</p>
              </div>
              <div className="bg-white px-2 py-3 text-center sm:px-4 sm:py-4">
                <p className="text-lg font-bold text-gray-900 sm:text-xl">{profile.followers.toLocaleString()}</p>
                <p className="text-[10px] text-gray-500 sm:text-xs">Followers</p>
              </div>
              <div className="bg-white px-2 py-3 text-center sm:px-4 sm:py-4">
                <p className="text-lg font-bold text-gray-900 sm:text-xl">{profile.following.toLocaleString()}</p>
                <p className="text-[10px] text-gray-500 sm:text-xs">Following</p>
              </div>
            </div>
          </section>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  Loaded {repos.length.toLocaleString()} of {profile.public_repos.toLocaleString()} public repositories
                </p>
              </div>
            </div>
